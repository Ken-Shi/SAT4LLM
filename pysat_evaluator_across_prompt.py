...
def plot_bar_chart_with_line_and_annotations(L, L2, K, xlabels, model):
    ...
    bars1 = ax.bar(index - bar_width / 2, L, bar_width, color='purple', label='Single Answer')
    bars2 = ax.bar(index + bar_width / 2, L2, bar_width, color='orange', label='Line-of-Thought Answer')
    ...
    ax.set_title('GPT Model (' + model + ') Performance in solving SAT problems with different prompt styles')
    ax.set_xticks(index)
...
def check_cnf_satisfaction(cnf_file, assignment_str, N, prompt_type):
    # Load the CNF file
            ...
            for prompt_type in prompt_types:
                ...
                for filename in os.listdir(prompt_path + model + type_name):
                    if filename.endswith(".txt"):
                        ...
                        satisfied = check_cnf_satisfaction(cnf_path, solution, variable_bound, prompt_type)
                        #if not satisfied:
                        #    print(solution)
                        ...
                        if problem_count >= amount_problems:
                            break
                all_results[prompt_type][model].append(correct_count)
