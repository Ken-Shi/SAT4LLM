...
def plot_bar_chart_with_line_and_annotations(L, L2, K, xlabels):
    ...
    ax.set_title('GPT Model Performance in solving SAT problems with complex prompts')
    ax.set_xticks(index)
...
def check_cnf_satisfaction(cnf_file, assignment_str, N):
    # Load the CNF file
    if assignment_str == "SATISFIABLE" or assignment_str == "UNSATISFIABLE":
        return False
    assignment = parse_assignment(assignment_str, N)
    if len(assignment) == 0:
        return False
...
def parse_assignment(assignment_str, N):
    assignment = {}
        ...
        for model in models:
            ...
            for filename in os.listdir(prompt_path + model + type_name):
                if filename.endswith(".txt"):
                    ...
                    satisfied = check_cnf_satisfaction(cnf_path, solution, variable_bound)
                    #if not satisfied:
                    #    print(solution)
