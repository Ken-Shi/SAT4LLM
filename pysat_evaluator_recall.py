...
def find_answers(text):
    ...
    # This regex looks for 'answer:' optionally followed by any amount of whitespace,
    # and captures everything after that until the end of the line.
...
def plot_bar_chart_with_line_and_annotations(L, L2, K, xlabels, model):
    ...
    bars1 = ax.bar(index - bar_width / 2, L, bar_width, color='gold', label='Single Answer')
    bars2 = ax.bar(index + bar_width / 2, L2, bar_width, color='slategrey', label='Line-of-Thought Answer')
    ...
    ax.set_ylabel('Number of Problems Recalled')
    ax.set_title('GPT Model (' + model + ') Recall in solving SAT problems with different prompt styles')
    ax.set_xticks(index)
...
def check_cnf_satisfaction_recall(cnf_file, assignment_str, N, prompt_type):
    # Load the CNF file
    if prompt_type == "single-2":
        assignment = parse_assignment_standard(assignment_str)
    elif prompt_type == "complex":
        assignment = parse_assignment_complex(assignment_str, N)
    print(assignment)
    if len(assignment) == 0:
        return True
    else:
...
parse_literals = lambda s: s.split('=')[0] if s.split('=')[1] == 'True' else f"-{s.split('=')[0]}"
parse_literals_alt = lambda s: s.split('=')[0] if s.split('=')[1] == 'T' else f"-{s.split('=')[0]}"
parss_numerical = lambda s: s.split('=')[0] if s.split('=')[1] == '1' else f"-{s.split('=')[0]}"

def parse_assignment_standard(assignment_str):
    assignment = {}
...
def parse_assignment_complex(assignment_str, N):
    assignment = {}
    answer_statements = find_answers(assignment_str)
    items = []
    #if len(answer_statements) > 0:
    #    items = answer_statements[-1].split()
    for answer_statement in answer_statements:
        items = items + answer_statement.split()
    #print(items)
    new_items = []
    current_cnt = 1
    for item in items:
        if len(item) > 1 and (item[0] == '(' or item[0] == '{' or item[0] == 'x' or item[0] == "'"):
            item = item[1:]
        if len(item) > 1 and (item[-1] == ')' or item[-1] == '}' or item[-1] == ',' or item[-1] == '.'):
            item = item[:len(item) - 1]
        if len(item) > 1 and item[-1] == "'":
            item = item[:len(item) - 1]

        if "=True" in item or "=False" in item:
            item = parse_literals(item)

        if "=T" in item or "=F" in item:
            item = parse_literals_alt(item)

        if "=1" in item or "=-1" in item:
            item = parss_numerical(item)

        if item == "True":
            item = str(current_cnt)
            current_cnt += 1
...
if __name__ == "__main__":
    ...
    all_results = {}
    all_results["single-2"] = {}
    all_results["single-2"]["gpt-3.5-turbo"] = []
    all_results["single-2"]["gpt-4"] = []
    all_results["complex"] = {}
    all_results["complex"]["gpt-3.5-turbo"] = []
    all_results["complex"]["gpt-4"] = []
    ...
    for entry in labels_to_include:
        ...
        for model in models:
            for prompt_type in prompt_types:
                ...
                correct_count = 0
                problem_count = 0
                print(prompt_path + model + type_name)
                for filename in os.listdir(prompt_path + model + type_name):
                    if filename.endswith(".txt"):
                        ...
                        satisfied = check_cnf_satisfaction_recall(cnf_path, solution, variable_bound, prompt_type)
                        #if not satisfied:
                        #    print(solution)
                        ...
                        if problem_count >= amount_problems:
                            break
                all_results[prompt_type][model].append(len(solution_results) - correct_count)
    ...
    for model in models:
        plot_bar_chart_with_line_and_annotations(all_results["single-2"][model], all_results["complex"][model], amount_problems, labels_to_include, model)
